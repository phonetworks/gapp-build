...
export default class Options extends React.Component {
    ...
    render() {
        ...
        if(scope) {
            ...
            let propertyList = scope.category == 'node' || scope.category == 'graph' ? [
                {
                    label: 'expires',
            ...
            return(
                <aside id="options" className={this.props.option ? 'open' : ''}>
                    ...
                    <Block label={(scope.category == 'graph' ? 'Main' : 'Active') + ' ' + scope.category.charAt(0).toUpperCase() + scope.category.substr(1)}  action="edit" color={color} callback={() => this.props.changeOptions('changeItem', [scope.category, scope.id])}>
